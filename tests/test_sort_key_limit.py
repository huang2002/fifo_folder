...
def test_sort_key_limit(test_path: Path) -> None:
    ...
    fifo_folder.sort_key_limit = 1023
    removed_items = fifo_folder.manage()
    ...
    fifo_folder.sort_key_limit = 512
    removed_items = fifo_folder.manage()
    assert len(removed_items) == 0
    assert len(fifo_folder.items) == 2
    assert os.path.basename(fifo_folder.items[0].data.path) == "0.txt"
    ...
    fifo_folder.sort_key_limit = 0
    removed_items = fifo_folder.manage()
    assert len(removed_items) == 2
    assert os.path.basename(removed_items[0].data.path) == "2.txt"
    assert os.path.basename(removed_items[1].data.path) == "0.txt"
